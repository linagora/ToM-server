...
```ts
import twakeConfig from './index.ts';
import { ConfigDescription } from './types.ts'; // Import ConfigDescription type
...
async function loadConfiguration() {
  try {
    ...
    const config2 = await twakeConfig(
...
Run `nx build @twake-chat/config-parser` to build the library.

## Running unit tests

Run `nx test @twake-chat/config-parser` to execute the unit tests via [Jest](https://jestjs.io).
