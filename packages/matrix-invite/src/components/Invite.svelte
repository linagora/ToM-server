<script lang="ts">
	...
	const handleSubmit = () => {
		...
		goto(`/invite/${encodeURIComponent(resource)}`);
	};

	const validator = (node: HTMLInputElement) => {
	...
	const setValidationMessage = () => {
		reference.setCustomValidity(
			"that doesn't seem valid, try #room:matrix.linagora.com, @user:martrix.linagora.com or +group:matrix.linagora.com"
		);
	};
</script>
...
<div class="bg-white rounded-xl p-8 pt-16 shadow-lg">
	<form class="flex flex-col space-y-6" onsubmit={preventDefault(handleSubmit)}>
		...
		<input
			...
			bind:this={reference}
			use:validator
			placeholder="#room:matrix.linagora.com, @user:matrix.linagora.com"
		...
		>
			...
			<span class="flex-grow items-center justify-center"> Create link </span>
		</button>
	</form>
</div>
