...
jest.mock('../controllers', () => {
  ...
  return function () {
    return {
      listAddressbook: passiveController,
      fetchContact: passiveController,
      addContacts: passiveController,
      updateContact: passiveController,
      deleteContact: passiveController,
      deleteAddressbook: passiveController
    }
  }
})
...
describe('the addressbook API router', () => {
  beforeAll((done) => {
    idServer.ready
      .then(() => {
        app.use(
  ...
  it('should not call the validation middleware if the Bearer token is not set', async () => {
    const response = await supertest(app).get(`${PATH}/contactId`)
