import MatrixClientServer from '@twake/matrix-client-server'
...
const matrixClientServer = new MatrixClientServer(conf)
const promises = [matrixClientServer.ready]
...
Promise.all(promises)
  .then(() => {
    Object.keys(matrixClientServer.api.get).forEach((k) => {
      app.get(k, matrixClientServer.api.get[k])
    })
    Object.keys(matrixClientServer.api.post).forEach((k) => {
      app.post(k, matrixClientServer.api.post[k])
    })
    const port = process.argv[2] != null ? parseInt(process.argv[2]) : 3000
    console.log(`Matrix Client-Server listening on port: ${port}`)
    app.listen(port)
  })
  .catch((e) => {
    throw new Error(e)
