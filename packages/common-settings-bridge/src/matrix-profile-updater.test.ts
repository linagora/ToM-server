...
describe("MatrixProfileUpdater", () => {
  ...
  beforeEach(() => {
    ...
    mockApis = {
      ...
      botUploadContent: jest.fn().mockResolvedValue("mxc://example.com/bot-uploaded"),
    };
  ...
  describe("updateDisplayName", () => {
    describe("DISABLED mode", () => {
      ...
      beforeEach(() => {
        updater = new MatrixProfileUpdater(mockApis, SynapseAdminRetryMode.DISABLED, mockLogger, {
      ...
      it("should use intent API successfully", async () => {
        ...
        expect(mockLogger.info).toHaveBeenCalledWith('Updated display name for @user:example.com to "John Doe"');
      });
      ...
      it("should throw error on failure without fallback", async () => {
        ...
        await expect(updater.updateDisplayName("@user:example.com", "John Doe")).rejects.toThrow("Network error");
      ...
      it("should throw on M_FORBIDDEN without fallback", async () => {
        ...
        await expect(updater.updateDisplayName("@user:example.com", "John Doe")).rejects.toMatchObject({
          errcode: "M_FORBIDDEN",
        });
    ...
    describe("EXCLUSIVE mode", () => {
      ...
      beforeEach(() => {
        updater = new MatrixProfileUpdater(mockApis, SynapseAdminRetryMode.EXCLUSIVE, mockLogger);
      });
      ...
      it("should use admin API only", async () => {
        ...
        expect(mockApis.adminUpsertUser).toHaveBeenCalledWith("@user:example.com", { displayname: "John Doe" });
      ...
      it("should throw error on admin API failure", async () => {
        ...
        await expect(updater.updateDisplayName("@user:example.com", "John Doe")).rejects.toThrow("Admin API error");
    ...
    describe("FALLBACK mode", () => {
      ...
      beforeEach(() => {
        updater = new MatrixProfileUpdater(mockApis, SynapseAdminRetryMode.FALLBACK, mockLogger, {
          maxSizeBytes: 5 * 1024 * 1024,
          fetchTimeoutMs: 10_000,
        });
      });
      ...
      it("should fall back to admin API on M_FORBIDDEN", async () => {
        ...
        expect(mockIntent.setDisplayName).toHaveBeenCalledWith("John Doe");
        expect(mockApis.adminUpsertUser).toHaveBeenCalledWith("@user:example.com", { displayname: "John Doe" });
        expect(mockLogger.info).toHaveBeenCalledWith("Falling back to admin API for display name @user:example.com");
        expect(mockLogger.info).toHaveBeenCalledWith(
          'Updated display name via admin API for @user:example.com to "John Doe"',
        );
      });
  ...
  describe("updateAvatar", () => {
    describe("MXC URL handling", () => {
      ...
      it("should use MXC URL directly without upload", async () => {
        ...
        expect(mockLogger.debug).toHaveBeenCalledWith(`Avatar URL is already MXC format: ${mxcUrl}`);
      });
    });
    ...
    describe("HTTP URL handling", () => {
      ...
      it("should upload HTTP URL via fetch and uploadContent", async () => {
        ...
        expect(mockMatrixClient.uploadContent).toHaveBeenCalledWith(expect.any(Buffer), "image/png", "avatar");
        expect(mockIntent.setAvatarUrl).toHaveBeenCalledWith(mxcUrl);
        expect(mockLogger.info).toHaveBeenCalledWith(
          `Downloading avatar from external URL for @user:example.com: ${httpUrl}`,
        );
        expect(mockLogger.info).toHaveBeenCalledWith(`Uploaded avatar to Synapse for @user:example.com: ${mxcUrl}`);
      });
      ...
      it("should propagate upload errors", async () => {
        ...
        await expect(updater.updateAvatar("@user:example.com", httpUrl)).rejects.toThrow("Upload failed");

        expect(mockIntent.setAvatarUrl).not.toHaveBeenCalled();
        expect(mockLogger.error).toHaveBeenCalledWith(`Failed to upload avatar for @user:example.com: Upload failed`);
      });
      ...
      it("should reject avatars exceeding size limit via content-length header", async () => {
        ...
        await expect(updater.updateAvatar("@user:example.com", httpUrl)).rejects.toThrow("Avatar too large");
      ...
      it("should timeout on slow avatar downloads", async () => {
        ...
        await expect(updatePromise).rejects.toThrow("Avatar fetch timed out after 10000ms");
      ...
      it("should handle HTTP errors from external URLs", async () => {
        ...
        await expect(updater.updateAvatar("@user:example.com", httpUrl)).rejects.toThrow("HTTP 404: Not Found");
    ...
    describe("DISABLED mode", () => {
      ...
      it("should throw on error without fallback", async () => {
        ...
        await expect(updater.updateAvatar("@user:example.com", mxcUrl)).rejects.toThrow("Network error");
    ...
    describe("EXCLUSIVE mode", () => {
      ...
      it("should use admin API only", async () => {
        ...
        expect(mockApis.adminUpsertUser).toHaveBeenCalledWith("@user:example.com", { avatar_url: mxcUrl });
        expect(mockIntent.setAvatarUrl).not.toHaveBeenCalled();
      });
      ...
      it("should upload HTTP URL avatar using bot credentials", async () => {
        ...
        (global.fetch as jest.Mock).mockResolvedValue(mockResponse);
        mockApis.botUploadContent.mockResolvedValue("mxc://example.com/bot-uploaded");
        mockApis.adminUpsertUser.mockResolvedValue(undefined);
        ...
        expect(mockApis.adminUpsertUser).toHaveBeenCalledWith("@user:example.com", {
    ...
    describe("FALLBACK mode", () => {
      ...
      it("should fall back to admin API on M_FORBIDDEN", async () => {
        ...
        expect(mockIntent.setAvatarUrl).toHaveBeenCalledWith(mxcUrl);
        expect(mockApis.adminUpsertUser).toHaveBeenCalledWith("@user:example.com", { avatar_url: mxcUrl });
        expect(mockLogger.info).toHaveBeenCalledWith("Falling back to admin API for avatar @user:example.com");
      });
      ...
      it("should throw on non-M_FORBIDDEN errors", async () => {
        ...
        await expect(updater.updateAvatar("@user:example.com", mxcUrl)).rejects.toMatchObject({ errcode: "M_UNKNOWN" });
      ...
      it("should fall back to bot upload on M_FORBIDDEN for HTTP URL", async () => {
        ...
        expect(mockIntent.setAvatarUrl).toHaveBeenCalledWith("mxc://example.com/bot-uploaded");
        expect(mockApis.adminUpsertUser).toHaveBeenCalledWith("@user:example.com", {
          avatar_url: "mxc://example.com/bot-uploaded",
        });
      });
  ...
  describe("processChanges", () => {
    ...
    const createPayload = (overrides?: Partial<ISettingsPayload>): ISettingsPayload => ({
      language: "en",
      timezone: "UTC",
    ...
    describe("display name changes", () => {
      it("should update display name when changed", async () => {
        ...
        await updater.processChanges("@user:example.com", oldPayload, newPayload);
      ...
      it("should not update display name when unchanged", async () => {
        ...
        expect(mockIntent.setDisplayName).not.toHaveBeenCalled();
        expect(mockLogger.debug).toHaveBeenCalledWith("No profile changes detected for @user:example.com");
      });
    ...
    describe("avatar changes", () => {
      it("should update avatar when changed", async () => {
        ...
        expect(mockIntent.setAvatarUrl).toHaveBeenCalledWith("mxc://example.com/new");
        expect(mockIntent.setDisplayName).not.toHaveBeenCalled();
      });
      ...
      it("should skip update when new avatar is empty", async () => {
        ...
        expect(mockIntent.setAvatarUrl).not.toHaveBeenCalled();
        expect(mockLogger.warn).toHaveBeenCalledWith("Avatar changed but new value is empty for @user:example.com");
      });
    });
    ...
    describe("new user handling", () => {
      it("should update both display name and avatar for new user", async () => {
        ...
        expect(mockIntent.setAvatarUrl).toHaveBeenCalledWith("mxc://example.com/avatar");
      });
