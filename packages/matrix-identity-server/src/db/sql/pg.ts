...
class Pg extends SQL implements IdDbBackend {
  ...
  createDatabases (conf: Config): Promise<boolean> {
    return new Promise((resolve, reject) => {
      import('pg').then(pg => {
        ...
        if (conf.database_host == null || conf.database_user == null || conf.database_password == null || conf.database_name == null) {
  ...
  exists (table: string): Promise<boolean> {
    return new Promise((resolve, reject) => {
      if (this.db != null) {
        this.db.query(`SELECT EXISTS (SELECT FROM pg_tables WHERE tablename='${table.toLowerCase()}')`, (err, res) => {
          if (err == null) {
  ...
  insert (table: string, values: Record<string, string | number>): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.db.query(`INSERT INTO ${table}(${names.join(',')}) VALUES(${names.map((v, i) => `$${i + 1}`).join(',')})`, vals, (err) => {
        // eslint-disable-next-line @typescript-eslint/strict-boolean-expressions
  ...
  update (table: Collections, values: Record<string, string | number>, field: string, value: string | number): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      if (this.db == null) {
        ...
        vals.push(value)
        this.db.query(`UPDATE ${table} SET ${names.map((name, i) => `${name}=$${i + 1}`).join(',')} WHERE ${field}=$${vals.length}`, vals, (err) => {
          // eslint-disable-next-line @typescript-eslint/strict-boolean-expressions
          err ? reject(err) : resolve()
        })
      }
      ...
      if (this.db == null) {
        ...
        if (field != null && value != null) {
          if (typeof value === 'object') {
            condition = 'WHERE ' + value.map((val, i) => `${field}=$${i + 1}`).join(' OR ')
          } else {
            value = [value as string]
            condition = 'WHERE ' + `${field}=$1`
          }
        }
        this.db.query(`SELECT ${fields.join(',')} FROM ${table} ${condition}`, value as string[], (err, rows) => {
          // eslint-disable-next-line @typescript-eslint/strict-boolean-expressions
