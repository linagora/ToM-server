...
const longKeyPair : { publicKey: string, privateKey: string, keyId: string }  = generateKeyPair('ed25519')
const shortKeyPair : { publicKey: string, privateKey: string, keyId: string }  = generateKeyPair('curve25519')
...
describe('Get public key from keyID', () => {
    ...
    it('should return the public key when correct keyID is given (from long term key pairs)', async () => {
      console.log("correct long")
        ...
        const response = await request(app)
          .get(`/_matrix/identity/v2/pubkey/${_keyID}`)
    ...
    it('should return 404 when incorrect keyID is given', async () => {
      console.log("incorrect")
        ...
        const response = await request(app)
          .get(`/_matrix/identity/v2/pubkey/${_keyID}`) // exactement '/_matrix/identity/v2/pubkey/' + _keyID
