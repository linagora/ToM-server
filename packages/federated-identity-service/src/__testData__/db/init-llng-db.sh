...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$DATABASE" <<-EOSQL
	...
	CREATE UNLOGGED TABLE IF NOT EXISTS $STABLE (
		id varchar(64) not null primary key,
		a_session jsonb
	);
	...
	CREATE UNLOGGED TABLE IF NOT EXISTS $SAMLTABLE (
	    id varchar(64) not null primary key,
	    a_session jsonb
	);
	CREATE INDEX IF NOT EXISTS i_a__session_kind ON $SAMLTABLE ((a_session ->> '_session_kind'));
	CREATE INDEX IF NOT EXISTS i_a__utime        ON $SAMLTABLE ((cast(a_session ->> '_utime' as bigint)));
	CREATE INDEX IF NOT EXISTS i_a_ProxyID       ON $SAMLTABLE ((a_session ->> 'ProxyID'));
	CREATE INDEX IF NOT EXISTS i_a__nameID       ON $SAMLTABLE ((a_session ->> '_nameID'));
	CREATE INDEX IF NOT EXISTS i_a__assert_id    ON $SAMLTABLE ((a_session ->> '_assert_id'));
	CREATE INDEX IF NOT EXISTS i_a__art_id       ON $SAMLTABLE ((a_session ->> '_art_id'));
	CREATE INDEX IF NOT EXISTS i_a__saml_id      ON $SAMLTABLE ((a_session ->> '_saml_id'));
	GRANT ALL PRIVILEGES ON TABLE $SAMLTABLE TO $USER;

	CREATE UNLOGGED TABLE IF NOT EXISTS $OIDCTABLE (
	    id varchar(64) not null primary key,
	    a_session jsonb
	);
	CREATE INDEX IF NOT EXISTS i_o__session_kind ON $OIDCTABLE ((a_session ->> '_session_kind'));
	CREATE INDEX IF NOT EXISTS i_o__utime        ON $OIDCTABLE ((cast(a_session ->> '_utime' as bigint )));
	GRANT ALL PRIVILEGES ON TABLE $OIDCTABLE TO $USER;

	CREATE UNLOGGED TABLE IF NOT EXISTS $CASTABLE (
	    id varchar(64) not null primary key,
	    a_session jsonb
	);
	CREATE INDEX IF NOT EXISTS i_c__session_kind ON $CASTABLE ((a_session ->> '_session_kind'));
	CREATE INDEX IF NOT EXISTS i_c__utime        ON $CASTABLE ((cast(a_session ->> '_utime' as bigint)));
	CREATE INDEX IF NOT EXISTS i_c__cas_id       ON $CASTABLE ((a_session ->> '_cas_id'));
	CREATE INDEX IF NOT EXISTS i_c_pgtIou        ON $CASTABLE ((a_session ->> 'pgtIou'));
