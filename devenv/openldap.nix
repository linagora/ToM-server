...
let
  ...
  mkUserEntry = user: ''
    ...
    cn: ${user.cn}
    sn: ${user.sn}
    ${optAttr "twakeDisplayName" user.cn}${optAttr "twakeFullname" user.cn}${
      optAttr "twakeEmails" (
        lib.optionalString (user ? mail && user.mail != null && user.mail != "") (twakeEmails user.mail)
      )
    }${
      optAttr "twakePhones" (
        lib.optionalString (user ? mobile && user.mobile != null && user.mobile != "") (
          twakePhones user.mobile
        )
      )
    }${optAttr "twakeWorkspaceUrl" (user.workspaceUrl or "")}twakeAccountStatus: ${
      if (user.active or true) then "active" else "disabled"
    }
    twakeCreatedAt: ${createdAt}
    ${
      optAttr "twakeIsTechnical" (if (user.isTechnical or false) then "TRUE" else "")
