...
import { beforeEach, describe, expect, it, mock, test, type Mock } from 'bun:test'
import { _search } from '../_search'
import { send, isMatrixId } from '../../../../utils'

// Mock @twake/utils
mock.module(resolve(import.meta.dir, '../../../../utils/index'), () => ({
  send: mock(),
  errMsg: mock((msg) => ({ error: msg })),
  toMatrixId: mock((uid, server) => `@${uid}:${server}`),
  isMatrixId: mock((id) => {
    if (!id || typeof id !== 'string') return false
    return /^@[^:]+:[^:]+$/.test(id)
  }),
  getLocalPart: mock((mxid) => {
    if (!mxid) return null
...
const mockAddressbookList = mock()
const mockUserInfoGetBatch = mock()
...
describe('_search factory', () => {
  ...
  const idServerMock = {
    userDB: {
      match: mock().mockResolvedValue([{ uid: 'drwho' }])
    },
    db: {},
    matrixDb: {
      db: {
        match: mock().mockResolvedValue([{ user_id: '@drwho:server' }])
      }
    },
    conf: {
      server_name: 'server',
  ...
  beforeEach(() => {
    ...
    ;(isMatrixId as Mock<(...args: any[]) => any>).mockImplementation((id) => {
      if (!id || typeof id !== 'string') return false
      return /^@[^:]+:[^:]+$/.test(id)
    })
  })
  ...
  describe('Basic search functionality', () => {
    it('should return matches and inactive matches successfully', async () => {
      ...
      const response = (send as Mock<(...args: any[]) => any>).mock.calls[0][2]
      expect(
  ...
  describe('MatrixDb integration', () => {
    it('should convert non-Matrix IDs to Matrix IDs', async () => {
      const serverWithLocalId = {
        ...
        matrixDb: {
          db: {
            match: mock().mockResolvedValue([{ user_id: 'localuser' }])
          }
        }
      }
    ...
    it('should handle matrixDb errors gracefully', async () => {
      const brokenMatrixServer = {
        ...
        matrixDb: {
          db: {
            match: mock().mockRejectedValue(new Error('MatrixDb error'))
          }
        }
      }
  ...
  describe('UserDB integration', () => {
    ...
    it('should handle userDB errors gracefully', async () => {
      const brokenUserServer = {
        ...idServerMock,
        userDB: {
          match: mock().mockRejectedValue(new Error('UserDB error'))
        }
      }
  ...
  describe('AddressBook integration', () => {
    ...
    it('should filter out invalid Matrix IDs from addressbook', async () => {
      ...
      const emptyDbServer = {
        ...idServerMock,
        userDB: { match: mock().mockResolvedValue([]) },
        matrixDb: { db: { match: mock().mockResolvedValue([]) } }
      }
  ...
  describe('Active vs Inactive user distinction', () => {
    ...
    it('should mark users only in userDB as inactive', async () => {
      const serverWithInactiveUser = {
        ...
        matrixDb: {
          db: {
            match: mock().mockResolvedValue([{ user_id: '@active:server' }])
          }
        },
        userDB: {
          match: mock()
  ...
  describe('Concurrent execution', () => {
    it('should continue if one data source fails', async () => {
      const mixedServer = {
        ...
        matrixDb: {
          db: {
            match: mock().mockRejectedValue(new Error('MatrixDb failed'))
          }
        },
        userDB: {
          match: mock().mockResolvedValue([{ uid: 'success' }])
        }
      }
