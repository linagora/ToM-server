...
class CronTasks<T extends string = never> {
  ...
  constructor(conf: Config, db: IdentityServerDb<T>, userDB: UserDB, logger: TwakeLogger) {
    this.tasks = [];
      ...
      const updateUsersTask = cron.schedule(
        ...
        () => {
          updateUsers(conf, db, userDB, logger)
            .then((result) => {
              if (result.success) logger.debug("Users update succeeded")
              else logger.error(`Users update failed: ${result.error}`)
            })
            .catch((e) => {
              // istanbul ignore next
  ...
  private readonly _addUpdateFederatedIdentityHashesJob = async (
    ...
    const cronString: string = conf.update_federated_identity_hashes_cron ?? "3 3 3 * * *";
    ...
    const task = cron.schedule(
      ...
      () => {
        updateFederatedIdentityHashes(conf, userDB, logger)
          .then(() => logger.debug("Federated identity hashes update succeeded"))
          .catch((e: Error) => {
            // istanbul ignore next
