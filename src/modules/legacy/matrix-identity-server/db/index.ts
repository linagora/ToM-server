import { generateKeyPair, randomString } from '../../crypto/index';
import type { TwakeLogger } from '../../logger/index';
import { epoch } from '../../utils/index';
import type { Config, DbGetResult, invitationToken } from "../types";
...
const tables: Record<Collections, string> = {
  ...
  hashes: "hash varchar(48) PRIMARY KEY, pepper varchar(32), type varchar(8), value text, active integer",
  invitationTokens: "id varchar(64) PRIMARY KEY, address text, data text",
  keys: "name varchar(32) PRIMARY KEY, data text",
  longTermKeypairs: "name text PRIMARY KEY, keyID varchar(64), public text, private text",
  mappings:
  ...
  shortTermKeypairs: "keyID varchar(64) PRIMARY KEY, public text, private text, active integer",
...
const initializeValues: Partial<Record<Collections, Array<Record<string, string | number>>>> = {
  keys: [
    { name: "pepper", data: "" },
...
class IdentityServerDb<T extends string = never> implements IdDbBackend<Collections | T> {
  ready: Promise<void>;
  ...
  constructor(
    ...
    additionnalInitializeValues?: Partial<Record<Collections, Array<Record<string, string | number>>>>,
  ) {
    ...
    this.db = new Module<Collections | T>(
      ...
      allInitializeValues as Partial<Record<Collections | T, Array<Record<string, string | number>>>>,
    );
  ...
  // eslint-disable-next-line @typescript-eslint/explicit-function-return-type, @typescript-eslint/promise-function-async
  update(table: Collections | T, values: Record<string, string | number>, field: string, value: string | number) {
  ...
  // eslint-disable-next-line @typescript-eslint/explicit-function-return-type, @typescript-eslint/promise-function-async
  getCount(table: Collections | T, field: string, value?: string | number | string[]) {
  ...
  // eslint-disable-next-line @typescript-eslint/explicit-function-return-type, @typescript-eslint/promise-function-async
  match(table: Collections | T, fields: string[], searchFields: string[], value: string | number) {
  ...
  // eslint-disable-next-line @typescript-eslint/explicit-function-return-type, @typescript-eslint/promise-function-async
  deleteLowerThan(table: Collections | T, field: string, value: string | number) {
    return this.db.deleteLowerThan(table, field, value);
  }

  // eslint-disable-next-line @typescript-eslint/explicit-function-return-type, @typescript-eslint/promise-function-async
  deleteWhere(table: Collections | T, conditions: ISQLCondition | ISQLCondition[]) {
  ...
  async createKeypair(type: "longTerm" | "shortTerm", algorithm: "ed25519" | "curve25519"): Promise<keyPair> {
    ...
    try {
      if (type === "longTerm") {
        ...
        if (currentKey.length > 0) {
          ...
          await this.db.update("longTermKeypairs", { name: "previousKey" }, "name", "currentKey");
        }
      }
    } catch (error) {
    ...
    return new Promise((resolve) => {
      this.db
        .insert(type === "longTerm" ? "longTermKeypairs" : "shortTermKeypairs", {
          keyID: keyPair.keyId,
