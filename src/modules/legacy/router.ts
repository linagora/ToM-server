...
import FederatedIdentityService from "./federated-identity-service/index";
import { legacyConfigSchema } from "./schema";
import TomServer from "./tom-server/index";
...
async function mountFederatedRouter(config: LegacyConfig, logger: Logger): Promise<Router> {
  const fedServer = new FederatedIdentityService(
    config,
    undefined,
    logger.child({
      module: "Federated Legacy",
    }),
  );
...
// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: Simple mapping function, no logic branches
...
export async function createLegacyRouter(config: Config, logger: Logger): Promise<Router> {
  const legacyRouter = await initializeLegacyRoutes(
